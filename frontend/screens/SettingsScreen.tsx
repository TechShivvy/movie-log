...
function ThemeSwatch({
  ...
  t: { key: string; label: string; bg: string; surface: string; accent: string };
  active: boolean;
  onSelect: () => void;
}) {
  if (Platform.OS === "web") {
    return (
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" } as React.CSSProperties}>
          <span style={{ fontSize: 14, color: "var(--color-text)", flex: 1 } as React.CSSProperties}>{t.label}</span>
          {active && <CheckCircle size={16} color="var(--color-accent)" weight="fill" />}
        </div>
      </div>
    );
  }
  ...
  return (
    <Pressable
      ...
      style={{
        borderRadius: 8,
        borderWidth: 1,
        borderColor: active ? undefined : undefined, // set below
        padding: 12,
      }}
    >
      {({ pressed }) => {
        ...
        return (
          <View style={{
            borderRadius: 8,
            borderWidth: 1,
            borderColor: active ? cur.accent : cur.divider,
            backgroundColor: active ? cur.accent900 : "transparent",
            padding: 12,
            gap: 8,
          }}>
            ...
            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
              <Text style={{ fontSize: 14, color: cur.text, flex: 1 }}>{t.label}</Text>
              {active && <CheckCircle size={16} color={cur.accent} weight="fill" />}
            </View>
          </View>
        );
      }}
...
// ─── Native theme swatch (simpler, no nested hook) ───────────────────────────
