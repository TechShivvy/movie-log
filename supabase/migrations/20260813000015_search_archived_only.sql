-- Completes Phase 1 of plan.md: search_movie_logs needs the same
-- "archived logs are cold storage, hidden from the caller's own default
-- view too" treatment GET /movie-logs just got. New parameter inserted
-- rather than appended -- DROP first, same reason every prior signature
-- change to this function has needed it.
