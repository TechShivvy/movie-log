...
import {
  setTheme,
  setAutoFill as setAF,
  setPreferredModel as setPM,
    ...
    useCallback(() => {
      (async () => {
        ...
        const { data } = await supabase
          ...
          .maybeSingle();
        if (data) {
          dispatch(setAF(Boolean(data.auto_fill)));
          dispatch(setPM(data.preferred_model || MODELS[0]));
        }
        const k = await getUserOpenRouterKey();
        setOwnKey(k || "");
      })();
    }, []),
  );
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.sub, { color: c.textSecondary }]}>
        Manage your cinema collection and app experience.
      </Text>
      ...
      <GlassCard style={styles.section}>
        ...
        <View style={styles.swatchGrid}>
          {THEME_OPTIONS.map((t) => (
            <Pressable
              key={t.name}
              onPress={() => dispatch(setTheme(t.name as ThemeName))}
              style={[
                styles.swatch,
                {
                  backgroundColor: c.surfaceElevated,
                  borderColor:
                    themeName === t.name ? CM.primaryContainer : c.border,
                },
                themeName === t.name && styles.swatchActive,
              ]}
            >
      ...
      <GlassCard style={styles.section}>
        <View style={styles.row}>
          ...
          <Switch
            ...
            thumbColor={CM.primaryContainer}
            trackColor={{ true: CM.primaryContainer + "55", false: c.border }}
          />
        </View>
      </GlassCard>
      ...
      <GlassCard style={styles.section}>
        ...
        {MODELS.map((m) => (
          <Pressable
            key={m}
            style={[styles.radioRow, m === model && styles.radioActive]}
            onPress={() => dispatch(setPM(m))}
          >
            <Ionicons
              name={m === model ? "radio-button-on" : "radio-button-off"}
              size={18}
              color={m === model ? CM.primaryContainer : c.textSecondary}
            />
            <Text
              style={[
                styles.radioText,
                { color: m === model ? c.textPrimary : c.textSecondary },
              ]}
            >
              {m}
            </Text>
      ...
      <Field
        ...
        secureTextEntry
        icon="key-outline"
        hint="Bypasses daily shared limit."
      />
      <Button
        label="Save settings"
        fullWidth
      ...
      {/* Dev / token */}
      <GlassCard
        style={{ ...styles.section2, borderColor: CM.tertiaryContainer + "44" }}
      >
        <View style={styles.devHeader}>
          <Ionicons
            name="code-slash-outline"
            size={16}
            color={CM.tertiaryContainer}
          />
          <Text style={[styles.sectionTitle, { color: CM.tertiaryContainer }]}>
            API / Swagger Access
          </Text>
        </View>
        <Text style={[styles.devBody, { color: c.textSecondary }]}>
          Copy token → Swagger Authorize (Bearer) at{" "}
      ...
      <GlassCard style={{ ...styles.section2, borderColor: CM.error + "44" }}>
        <View style={styles.devHeader}>
          ...
          <Text style={[styles.sectionTitle, { color: CM.error }]}>
            Danger Zone
          </Text>
        </View>
        <Text style={[styles.devBody, { color: c.textSecondary }]}>
...
const styles = StyleSheet.create({
  ...
  sub: { ...typography.bodyLg, marginTop: -spacing.md },
  section: { rowGap: spacing.md },
  section2: { rowGap: spacing.md },
